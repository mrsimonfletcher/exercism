class Game
  ...
  def roll(pins)
    if pins < 0 || pins > 10
      raise BowlingError.new("Pins must have a value from 0 to 10")
    elsif pins > @pins_up
      raise BowlingError.new("Pin count exceeds pins on the lane")
    elsif @frame_number == 10
      raise BowlingError.new("Should not be able to roll after game is over")
    end
  ...
  def score
    ...
    when 9
      raise BowlingError.new("Game is not yet over, cannot score!")
    else
      raise BowlingError.new("Score cannot be taken until the end of the game")
    end
  end
